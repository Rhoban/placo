...
/**
 * @brief Represents an object (like a task or a constraint) that is
 * prioritized.
 */
class Prioritized {
public:
  ...
  enum Priority {
    /**
     * @brief Task that **has** to be enforced, will result in equality
     * constraint,
     */
    Hard = 0,
    /**
     * @brief Task that can be unenforced, will result in an objective function
     * (can be weighted)
     */
    Soft = 1,
    /**
     * @brief This is similar to ``Hard``, but with a scaling factor (that is
   ...
   * @param weight task weight
   */
  void configure(std::string name, Priority priority = Soft,
                 double weight = 1.0);
