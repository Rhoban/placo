...
/**
 * @brief A collection of parameters that can be used to define the capabilities
 * and the constants behind planning and control of an humanoid robot.
 *
...
public:
  ...
  /**
   * @brief Checks if the walk resulting from those parameters will have double
   * supports
   */
  ...
  /**
   * @brief Target offset for the ZMP x reference trajectory in the foot frame
   * [m]
   */
  ...
  enum FootstepClipping { Ellipsoid = 0, Box = 1, Conic = 2 };

  /**
   * @brief Applies the ellipsoid (L2) clipping to a given step size (dx, dy,
   * dtheta)
   */
   ...
   */
  Eigen::Vector3d ellipsoid_overlap_clip(HumanoidRobot::Side support_side,
                                         Eigen::Vector3d step);

  /**
   * @brief Clips a step using ellipsoid and overlap avoidance
   */
  Eigen::Vector3d box_overlap_clip(HumanoidRobot::Side support_side,
                                   Eigen::Vector3d step);

  /**
   * @brief Clips a step using ellipsoid and overlap avoidance
   */
  Eigen::Vector3d conic_overlap_clip(HumanoidRobot::Side support_side,
                                     Eigen::Vector3d step);

  /**
   * @brief Frames for opposite and neutral positions
   */
  Eigen::Affine3d opposite_frame(HumanoidRobot::Side side,
                                 Eigen::Affine3d T_world_foot, double d_x = 0.,
                                 double d_y = 0., double d_theta = 0.);
  Eigen::Affine3d neutral_frame(HumanoidRobot::Side side,
                                Eigen::Affine3d T_world_foot, double d_x = 0.,
                                double d_y = 0., double d_theta = 0.);
...
protected:
   ...
   * @brief Clips a step using ellipsoid and overlap avoidance
   */
  Eigen::Vector3d overlap_clip(HumanoidRobot::Side support_side,
                               Eigen::Vector3d step, FootstepClipping clipping);
};
} // namespace placo::humanoid