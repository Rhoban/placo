...
public:
  ...
  struct Trajectory {
    Trajectory();
    Trajectory(double com_target_z, double t_start = 0.,
               double trunk_pitch = 0., double trunk_roll = 0.);
    ...
    // A part of the trajectory contain a support and the associated
    // trajectories (CoM, swing foot)
    ...
    /**
     * @brief Returns the support corresponding to the given time in the
     * trajectory
     */
    FootstepsPlanner::Support get_support(double t);

    /**
     * @brief Returns the nth next support corresponding to the given time in
     * the trajectory. If n is greater than the number of remaining supports,
     * the last support is returned
     */
    FootstepsPlanner::Support get_next_support(double t, int n = 1);

    /**
     * @brief Returns the nth previous support corresponding to the given time
     * in the trajectory. If n is greater than the number of previous supports,
     * the first support is returned
     */
    ...
    /**
     * @brief Applies a given transformation to the left of all values issued by
     * the trajectory
     */
    void apply_transform(Eigen::Affine3d T);

    /**
     * @brief Returns the start time of the trajectory part corresponding to the
     * given time
     */
    double get_part_t_start(double t);

    /**
     * @brief Returns the end time of the trajectory part corresponding to the
     * given time
     */
    double get_part_t_end(double t);

    /**
     * @brief Returns the DCM at the end of the trajectory part corresponding to
     * the given time
     */
  ...
  protected:
    ...
    /**
     * @brief WalkPatternGenerator is allowed to access the protected fields in
     * the trajectory when building it
     */
  ...
  /**
   * @brief Plans a walk trajectory following given footsteps based on the
   * parameters of the WPG
  ...
  Trajectory plan(std::vector<FootstepsPlanner::Support> &supports,
                  Eigen::Vector3d initial_com_world, double t_start = 0.);

  /**
   * @brief Updates the walk trajectory to follow given footsteps based on the
   * parameters of the WPG.
   ...
   * @param t_replan The time (in the original trajectory) where the replan
   * happens
   * @return Updated trajectory
   */
  Trajectory replan(std::vector<FootstepsPlanner::Support> &supports,
  ...
  /**
   * @brief Replans the supports for a given trajectory given a footsteps
   * planner.
   */
  std::vector<FootstepsPlanner::Support>
  replan_supports(FootstepsPlanner &planner, Trajectory &trajectory,
   ...
   */
  std::vector<FootstepsPlanner::Support>
  update_supports(double t, std::vector<FootstepsPlanner::Support> supports,
   ...
   */
  Eigen::Vector2d get_optimal_zmp(Eigen::Vector2d world_dcm_start,
                                  Eigen::Vector2d world_dcm_end,
                                  double duration,
                                  FootstepsPlanner::Support &support);
...
protected:
  ...
  // The parameters to use for planning. The values are forwarded to the
  // relevant solvers when needed.
  ...
  /**
   * @brief Constrains the LIPM to ensure that the ZMP stays in the support
   * polygon and that the CoM stops at the end of an end support.
   ...
   */
  void constrain_lipm(problem::Problem &problem, LIPM &lipm,
                      FootstepsPlanner::Support &support, double omega_2,
                      HumanoidParameters &parameters);

  /**
   * @brief Plans the CoM trajectory for a given support. Returns false if the
   * QP solver failed to solve the problem.
   ...
   */
  bool plan_com(Trajectory &trajectory,
  ...
  void plan_sgl_support(Trajectory &trajectory, int part_index,
                        Trajectory *old_trajectory);
  void plan_feet_trajectories(Trajectory &trajectory,
                              Trajectory *old_trajectory = nullptr);

  /**
   * @brief Trims the old trajectory to the new time horizon without modifying
   * the CoM and feet trajectories.
