...
class WalkTasks {
public:
  void initialize_tasks(placo::kinematics::KinematicsSolver *solver,
                        placo::humanoid::HumanoidRobot *robot);
  void remove_tasks();
  virtual ~WalkTasks();

  void update_tasks(WalkPatternGenerator::Trajectory &trajectory, double t);
  void update_tasks(Eigen::Affine3d T_world_left, Eigen::Affine3d T_world_right,
  ...
  void reach_initial_pose(Eigen::Affine3d T_world_left, double feet_spacing,
                          double com_height, double trunk_pitch);
  ...
  // DCM error PID
  void update_tasks_and_pid(WalkPatternGenerator::Trajectory &trajectory,
                            double t, Eigen::Vector2d dcm, double omega,
                            double elapsed);
