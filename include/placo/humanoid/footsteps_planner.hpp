...
public:
  ...
  struct Footstep {
    ...
    static bool polygon_contains(std::vector<Eigen::Vector2d> &polygon,
  ...
  struct Support {
    ...
    /**
     * @brief The support side (you should call is_both() to be sure it's not a
     * double support before)
     */
    HumanoidRobot::Side side();
    ...
    /**
     * @brief Apply a transformation to a support (applied to all the footstep
     * frames)
     */
   ...
   */
  std::vector<Footstep> plan(HumanoidRobot::Side flying_side,
   ...
   */
  static std::vector<Support> make_supports(std::vector<Footstep> footsteps,
                                            double t_start, bool start = true,
                                            bool middle = false,
                                            bool end = true);
   ...
   */
  Footstep opposite_footstep(Footstep footstep, double d_x = 0.,
                             double d_y = 0., double d_theta = 0.);

  /**
   * @brief Same as opposite_footstep(), but the clipping is applied
   */
  Footstep clipped_opposite_footstep(Footstep footstep, double d_x = 0.,
                                     double d_y = 0., double d_theta = 0.);

  Footstep create_footstep(HumanoidRobot::Side side,
                           Eigen::Affine3d T_world_foot);

  // Humanoid parameters for planning and control
  HumanoidParameters &parameters;

  HumanoidParameters::FootstepClipping footstep_clipping =
      HumanoidParameters::FootstepClipping::Conic;

protected:
  virtual void plan_impl(std::vector<Footstep> &,
