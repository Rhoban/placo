...
/**
 * @brief LIPM is an helper that can be used to build problem involving LIPM
 * dynamics. The decision variables introduced here are jerks, which is
 * piecewise constant.
 */
class LIPM {
public:
  ...
  LIPM();
  LIPM(problem::Problem &problem, double dt, int timesteps, double t_start,
       LIPM &previous);
  LIPM(problem::Problem &problem, double dt, int timesteps, double t_start,
  ...
  static LIPM build_LIPM_from_previous(problem::Problem &problem, double dt,
                                       int timesteps, double t_start,
                                       LIPM &previous);
  ...
  /**
   * @brief Compute the natural frequency of a LIPM given its height (omega =
   * sqrt(g / h))
   */
