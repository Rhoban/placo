...
#include "placo/dynamics/contacts.hpp"
#include "placo/dynamics/frame_task.hpp"
#include "placo/dynamics/gear_task.hpp"
#include "placo/dynamics/joints_task.hpp"
#include "placo/dynamics/orientation_task.hpp"
#include "placo/dynamics/position_task.hpp"
#include "placo/dynamics/relative_frame_task.hpp"
#include "placo/dynamics/relative_orientation_task.hpp"
#include "placo/dynamics/relative_position_task.hpp"
...
// Constraints
#include "placo/dynamics/avoid_self_collisions_constraint.hpp"
...
public:
   ...
   * @pyignore
   */
  PositionTask &add_position_task(model::RobotWrapper::FrameIndex frame_index,
   ...
   * @return position task
   */
  PositionTask &add_position_task(std::string frame_name,
                                  Eigen::Vector3d target_world);
   ...
   */
  OrientationTask &
  add_orientation_task(model::RobotWrapper::FrameIndex frame_index,
   ...
   * @return orientation task
   */
  OrientationTask &add_orientation_task(std::string frame_name,
                                        Eigen::Matrix3d R_world_frame);

  /**
   * @brief Adds a frame task, which is a pseudo-task packaging position and
   * orientation, resulting in a decoupled task
   * @param frame_index target frame
   * @param T_world_frame target transformation in the world
   * @return frame task
   * @pyignore
   */
  FrameTask add_frame_task(model::RobotWrapper::FrameIndex frame_index,
   ...
   * @return frame task
   */
  FrameTask add_frame_task(std::string frame_name,
                           Eigen::Affine3d T_world_frame);
  ...
  add_relative_position_task(model::RobotWrapper::FrameIndex frame_a_index,
   ...
   */
  RelativePositionTask &
  add_relative_position_task(std::string frame_a_name, std::string frame_b_name,
                             Eigen::Vector3d target_world);
  ...
  add_relative_orientation_task(model::RobotWrapper::FrameIndex frame_a_index,
                                model::RobotWrapper::FrameIndex frame_b_index,
   ...
   */
  RelativeOrientationTask &
  add_relative_orientation_task(std::string frame_a_name,
                                std::string frame_b_name,
                                Eigen::Matrix3d R_a_b);

  /**
   * @brief Adds a relative frame task, which is a pseudo-task packaging
   ...
   */
  RelativeFrameTask
  add_relative_frame_task(model::RobotWrapper::FrameIndex frame_a_index,
                          model::RobotWrapper::FrameIndex frame_b_index,
   ...
   * @return relative frame task
   */
  RelativeFrameTask add_relative_frame_task(std::string frame_a_name,
                                            std::string frame_b_name,
                                            Eigen::Affine3d T_a_b);
  ...
  /**
   * @brief Adds a gear task, allowing replication of a joint. This can be used
   * to implement timing belt, if coupled with an internal force.
   * @return gear task
   */
  ...
  /**
   * @brief Adds a planar contact, which is unilateral in the sense of the local
   * body z-axis
  ...
  /**
   * @brief Adds a unilateral line contact, which is unilateral in the sense of
   * the local body z-axis
   * @param frame_task associated frame task
   ...
   */
  ExternalWrenchContact &add_external_wrench_contact(
  ...
  add_external_wrench_contact(std::string frame_name,
                              std::string reference = "world");

  /**
   * @brief Adds a puppet contact, this will add some free contact forces for
   * the whole system, allowing it to be controlled freely
  ...
  /**
   * @brief Sets the "safe" Qdd acceptable for a given joint (used by joint
   * limits)
   * @param joint
   * @param qdd
   */
  ...
  /**
   * @brief Sets the allowed torque limit by the solver for a given joint. This
   * will not affect the robot's model effort limit. When computing the velocity
   * vs torque limit, the robot's model effort will still be used. You can see
   * this limit as a continuous limit allowable for the robot, while the robot's
   * model limit is the maximum possible torque.
   * @param joint
   * @param limit
   */
   ...
   */
  template <typename T> T &add_contact(T *contact) {
    contact->solver = this;
