...
struct ManipulabilityTask : public Task {
  enum Type { POSITION = 0, ORIENTATION = 1, BOTH = 2 };

  ManipulabilityTask(model::RobotWrapper::FrameIndex frame_index, Type type,
                     double lambda_ = 1.0);
  ...
  /**
   * @brief Should the manipulability be minimized (can be useful to find
   * singularities)
   */
  bool minimize = false;
