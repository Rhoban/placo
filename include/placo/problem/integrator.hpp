...
/**
 * @brief Integrator can be used to efficiently build expressions and values
 * over a decision variable that is integrated over time with a given linear
 * system.
 */
class Integrator {
public:
  /**
   * @brief The trajectory can be detached after a solve to retrieve the
   * continuous trajectory produced by the solver.
   */
  struct Trajectory {
    /**
     * @brief Gets the value of the trajectory at a given time and
     * differentiation
     * @param t time
  ...
  /**
   * @brief Creates an integrator able to build expressions and values over a
   * decision variable. With this constructor, a continuous system matrix will
   * be used (see below)
  ...
  /**
   * @brief Creates an integrator able to build expressions and values over a
   * decision variable. A custom continuous system matrix is passed, so that:
   *
   *        dX = MX
   *
   ...
   * @pyignore
   */
  Integrator(Variable &variable, Expression X0, Eigen::MatrixXd system_matrix,
             double dt);

  virtual ~Integrator();

  /**
   * @brief Builds a matrix M so that the system differential equation is dX = M
   * X
   *
  ...
  AB_matrices(Eigen::MatrixXd &M, int order, double dt);

  /**
   * @brief Builds an expression for the given step and differentiation
   * @param step the step, (if -1 the last step will be used)
   * @param diff differentiation (if -1, the expression will be a vector of size
...
protected:
  /**
   * @brief The last variable version that was used to build the internal
   * trajectory
   */
  int version = 0;
