...
/**
 * @brief A problem is an object that has variables and constraints to be solved
 * by a QP solver.
 */
class Problem {
public:
   ...
   *
   * If \ref rewrite_equalities is true, this should be equals to \ref
   * n_variable.
   */
  ...
  /**
   * @brief If set to true, some sparsity optimizations will be performed when
   * building the problem Hessian. This optimization is generally not useful for
   * small problems.
   */
  bool use_sparsity = true;

  /**
   * @brief If set to true, a QR factorization will be performed on the equality
   * constraints, and the QP will be called with free variables only.
   *
   * The number of free variables will be available in \ref free_variables, and
   * the number of determined variables in \ref determined_variables.
   */
...
protected:
  ...
  /**
   * @brief Used internally to access a constraint expression, optionally
   * applying the change of basis imposed by the QR decomposition, see \ref
   * rewrite_equalities.
   * @param constraint constraint
   * @param A output matrix A
   * @param b output vector b
   */
  void get_constraint_expressions(ProblemConstraint *constraint,
