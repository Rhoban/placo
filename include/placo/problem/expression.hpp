...
/**
 * @brief An expression is a linear combination of decision variables of the
 * form Ax + b that can be conveniently manipulated using operators
 */
class Expression {
public:
  Expression();
  ...
  /**
   * @brief Builds an expression from a double (A will be zero, the expression
   * is only one row)
   * @param value value
   * @return expression
   */
  ...
  /**
   * @brief Retrieve the expression value, given a decision variable. This can
   * be used after a problem is solved to retrieve a specific expression value.
   *
   * For exemple, passing ``problem.x`` after calling \ref Problem::solve will
  ...
  /**
   * @brief checks if the expression is constant (doesn't depend on decision
   * variables)
