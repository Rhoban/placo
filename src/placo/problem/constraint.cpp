...
void ProblemConstraint::configure(ProblemConstraint::Priority priority_,
                                  double weight_) {
  priority = priority_;
  weight = weight_;
}
...
bool ProblemConstraint::operator==(const ProblemConstraint &other) const {
  return (expression.A == other.expression.A) &&
         (expression.b == other.expression.b) && (priority == other.priority) &&
