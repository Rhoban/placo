...
ProblemConstraint &Problem::add_limit(Expression expression,
  ...
  e.A.block(expression.A.rows(), 0, expression.A.rows(), expression.A.cols()) =
      -expression.A;
  e.b.block(expression.b.rows(), 0, expression.b.rows(), expression.b.cols()) =
      -expression.b;
...
ProblemConstraint &
Problem::add_constraint(const ProblemConstraint &constraint_) {
...
void Problem::get_constraint_expressions(ProblemConstraint *constraint,
                                         Eigen::MatrixXd &A,
                                         Eigen::MatrixXd &b) {
  if (determined_variables) {
    Eigen::MatrixXd full_A(constraint->expression.A.rows(), n_variables);
    full_A.setZero();
    full_A.block(0, 0, constraint->expression.A.rows(),
                 constraint->expression.A.cols()) = constraint->expression.A;
...
void Problem::solve() {
  ...
  for (auto constraint : constraints) {
    if (constraint->type == ProblemConstraint::Equality &&
        constraint->priority == ProblemConstraint::Hard) {
      // Ax + b = 0
      A.block(k_equality, 0, constraint->expression.rows(),
              constraint->expression.cols()) = constraint->expression.A;
      b.block(k_equality, 0, constraint->expression.rows(), 1) =
          constraint->expression.b;
  ...
  if (rewrite_equalities && A.rows() > 0) {
    ...
    if (determined_variables != A.rows()) {
      throw QPError("QR decomposition failed to find a full rank matrix for "
                    "equality constraints");
    }

    Eigen::MatrixXd R = QR.matrixR().transpose().block(
        0, 0, determined_variables, determined_variables);
  ...
  Eigen::MatrixXd P(free_variables + slack_variables,
                    free_variables + slack_variables);
  ...
  // Scanning the constraints (counting inequalities and equalities, building
  // objectif function)
  for (auto constraint : constraints) {
    if (constraint->expression.cols() > n_variables) {
      throw QPError("Problem: Inconsistent problem size");
    }
    if (constraint->expression.A.rows() == 0 ||
        constraint->expression.b.rows() == 0) {
    ...
    if (constraint->type == ProblemConstraint::Inequality) {
      // If the constraint is hard, this will be the true inequality, else, this
      // will be the inequality enforcing the slack variable to be >= 0
      ...
      if (use_sparsity) {
        ...
        for (auto interval : sparsity.intervals) {
          ...
          Eigen::MatrixXd block =
              expression_A.block(0, interval.start, constraints, size);
  ...
  for (auto constraint : constraints) {
    if (constraint->type == ProblemConstraint::Inequality) {
      ...
      if (constraint->priority == ProblemConstraint::Hard) {
        // Ax + b >= 0
        G.block(k_inequality, 0, expression_A.rows(), expression_A.cols()) =
            expression_A;
        h.block(k_inequality, 0, expression_b.rows(), 1) = expression_b;

        for (int k = k_inequality; k < k_inequality + expression_A.rows();
             k++) {
        ...
        // min(Ax + b - s)
        // A slack variable is assigend with all "soft" inequality and a
        // minimization is added to the problem
        Eigen::MatrixXd As(expression_A.rows(),
                           free_variables + slack_variables);
        As.setZero();
  ...
  qp_x.setZero();
  double result =
      eiquadprog::solvers::solve_quadprog(P, q, A.transpose(), b, G.transpose(),
                                          h, qp_x, active_set, active_set_size);
  ...
  if (result == std::numeric_limits<double>::infinity()) {
    throw QPError(
        "Problem: Infeasible QP (check your hard inequality constraints)");
  }

  // Checking that equality constraints were enforced, since this is not covered
  // by above result
  if (A.rows() > 0) {
    ...
    for (int k = 0; k < A.rows(); k++) {
      if (fabs(equality_constraints[k]) > 1e-6) {
        throw QPError(
            "Problem: Infeasible QP (equality constraints were not enforced)");
      }
    }
  }
  ...
  for (int k = 0; k < active_set_size; k++) {
    ...
    if (active_constraint >= 0 &&
        hard_inequalities_mapping.count(active_constraint)) {
...
void Problem::dump_status() {
  ...
  if (rewrite_equalities) {
    std::cout << "  - Determined variables: " << determined_variables
              << std::endl;
