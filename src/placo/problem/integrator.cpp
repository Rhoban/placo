...
Integrator::Integrator(Variable &variable_, Expression X0,
                       Eigen::MatrixXd system_matrix, double dt)
...
Integrator::Integrator(Variable &variable_, Expression X0, int order, double dt)
  ...
  if (X0.rows() != order) {
    throw std::runtime_error("Integrator: X0 should have " +
                             std::to_string(order) + " rows (same as order)");
  }
}
...
Integrator::AB_matrices(Eigen::MatrixXd &M, int order, double dt) {
...
void Integrator::check_diff(int order, int diff, bool allow_all) {
  ...
  if (diff < diff_min || diff > order) {
    ...
    oss << "Asked differentiation order of " << diff
        << " for an integrator of order " << order;
...
Expression Integrator::expr(int step, int diff) {
  ...
  if (step < 0 || step > variable->size()) {
    std::ostringstream oss;
    oss << "Asking an expression for step " << step << ", should be between "
        << 0 << " and " << variable->size();
    throw std::runtime_error(oss.str());
  }
  ...
  if (diff == order) {
    ...
    if (diff == -1) {
      e.A.block(0, variable->k_start, rows, step) =
          final_transition_matrix.block(0, N - step, rows, step);
      e = e + a_powers[step] * X0;
    } else {
      e.A.block(0, variable->k_start, 1, step) =
...
void Integrator::update_trajectory() {
  if (variable->version == 0) {
    throw std::runtime_error(
        "Trying to get the trajectory with a variable that was not solved");
  }
