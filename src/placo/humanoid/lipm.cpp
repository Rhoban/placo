...
LIPM::LIPM(problem::Problem &problem, double dt, int timesteps, double t_start,
    ...
    : dt(dt), timesteps(timesteps), t_start(t_start) {
  x_var = &problem.add_variable(timesteps);
  y_var = &problem.add_variable(timesteps);
  x = Integrator(*x_var,
                 Eigen::VectorXd(Eigen::Vector3d(
                     initial_pos.x(), initial_vel.x(), initial_acc.x())),
                 3, dt);
  y = Integrator(*y_var,
                 Eigen::VectorXd(Eigen::Vector3d(
                     initial_pos.y(), initial_vel.y(), initial_acc.y())),
                 3, dt);
  x.t_start = t_start;
  y.t_start = t_start;
}
...
LIPM LIPM::build_LIPM_from_previous(problem::Problem &problem, double dt,
                                    int timesteps, double t_start,
                                    LIPM &previous) {
  return LIPM(problem, dt, timesteps, t_start, previous);
}

double LIPM::t_end() { return t_start + timesteps * dt; }
