...
FootstepsPlanner::Footstep::compute_polygon(double margin) {
  ...
  for (auto sxsy : contour) {
    Eigen::Vector3d corner =
        frame * Eigen::Vector3d(sxsy.first * (margin + foot_length / 2),
                                sxsy.second * (margin + foot_width / 2), 0.);
...
bool FootstepsPlanner::Footstep::polygon_contains(
...
Eigen::Affine3d
FootstepsPlanner::Support::footstep_frame(HumanoidRobot::Side side) {
  for (auto &footstep : footsteps) {
...
HumanoidRobot::Side FootstepsPlanner::Support::side() {
  if (footsteps.size() > 1) {
    throw std::runtime_error(
        "Asking side() for a double support (call is_both() before)");
  }

  return footsteps[0].side;
}

bool FootstepsPlanner::Support::is_both() { return footsteps.size() == 2; }

FootstepsPlanner::Support operator*(Eigen::Affine3d T,
                                    const FootstepsPlanner::Support &support) {
...
FootstepsPlanner::FootstepsPlanner(HumanoidParameters &parameters)
    : parameters(parameters) {}

std::vector<FootstepsPlanner::Support> FootstepsPlanner::make_supports(
    std::vector<FootstepsPlanner::Footstep> footsteps, double t_start,
  ...
  if (end) {
    // Creating the first (double-support) initial state
    Support support(
        {footsteps[footsteps.size() - 2], footsteps[footsteps.size() - 1]});
    support.end = true;
    supports.push_back(support);
  }
...
FootstepsPlanner::Footstep
FootstepsPlanner::create_footstep(HumanoidRobot::Side side,
                                  Eigen::Affine3d T_world_foot) {
  FootstepsPlanner::Footstep footstep(parameters.foot_width,
...
FootstepsPlanner::Footstep
FootstepsPlanner::opposite_footstep(FootstepsPlanner::Footstep footstep,
                                    double d_x, double d_y, double d_theta) {
  footstep.frame = parameters.opposite_frame(footstep.side, footstep.frame, d_x,
                                             d_y, d_theta);
...
FootstepsPlanner::Footstep
FootstepsPlanner::clipped_opposite_footstep(Footstep footstep, double d_x,
  ...
  } else if (footstep_clipping ==
             HumanoidParameters::FootstepClipping::Ellipsoid) {
  ...
  for (int k = 0; k < 32; k++) {
    Footstep new_footstep =
        opposite_footstep(footstep, step.x(), step.y(), step.z());
...
FootstepsPlanner::plan(HumanoidRobot::Side flying_side,
  ...
  auto T_world_current_frame = (current_side == HumanoidRobot::Side::Left)
                                   ? T_world_left
                                   : T_world_right;
  footsteps.push_back(create_footstep(current_side, T_world_current_frame));

  current_side = HumanoidRobot::other_side(current_side);
  T_world_current_frame = (current_side == HumanoidRobot::Side::Left)
                              ? T_world_left
                              : T_world_right;
  footsteps.push_back(create_footstep(current_side, T_world_current_frame));
