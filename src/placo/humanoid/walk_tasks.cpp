...
void WalkTasks::initialize_tasks(KinematicsSolver *solver_,
                                 HumanoidRobot *robot_) {
  robot = robot_;
  solver = solver_;

  left_foot_task =
      solver->add_frame_task("left_foot", robot->get_T_world_left());
  left_foot_task.configure("left_foot", scaled ? "scaled" : "soft", 1., 1.);

  right_foot_task =
      solver->add_frame_task("right_foot", robot->get_T_world_right());
  right_foot_task.configure("right_foot", scaled ? "scaled" : "soft", 1., 1.);

  trunk_orientation_task = &solver->add_orientation_task(
      "trunk", robot->get_T_world_trunk().rotation());
...
void WalkTasks::update_com_task() {
  if (trunk_mode) {
    ...
    if (trunk_task == nullptr) {
      trunk_task = &solver->add_position_task(
          "trunk", robot->get_T_world_frame("trunk").translation());
...
void WalkTasks::reach_initial_pose(Eigen::Affine3d T_world_left,
                                   double feet_spacing, double com_height,
                                   double trunk_pitch) {
  Eigen::Affine3d T_world_right = T_world_left;
  T_world_right.translation() =
      T_world_left.translation() +
      T_world_left.rotation() * Eigen::Vector3d(0, -feet_spacing, 0);

  Eigen::Vector3d com_world =
      interpolate_frames(T_world_left, T_world_right, .5).translation();
  com_world.z() = com_height;

  double trunk_yaw = pinocchio::rpy::matrixToRpy(T_world_left.rotation()).z();
  Eigen::Matrix3d R_world_trunk =
      pinocchio::rpy::rpyToMatrix(Eigen::Vector3d(0, trunk_pitch, trunk_yaw));
...
void WalkTasks::update_tasks(WalkPatternGenerator::Trajectory &trajectory,
                             double t) {
...
void WalkTasks::update_tasks(Eigen::Affine3d T_world_left,
                             Eigen::Affine3d T_world_right,
                             Eigen::Vector3d com_world,
                             Eigen::Matrix3d R_world_trunk) {
  update_com_task();
  Eigen::Vector3d offset = robot->get_T_world_frame("trunk").linear() *
                           Eigen::Vector3d(com_x, com_y, 0);
...
void WalkTasks::update_tasks_and_pid(
    WalkPatternGenerator::Trajectory &trajectory, double t, Eigen::Vector2d dcm,
    double omega, double elapsed) {
  Eigen::Vector2d error =
      dcm - trajectory.get_p_world_DCM(t + com_delay, omega);
  ...
  update_tasks(trajectory.get_T_world_left(t), trajectory.get_T_world_right(t),
               trajectory.get_p_world_CoM(t + com_delay) +
                   Eigen::Vector3d(com_offset[0], com_offset[1], 0.),
               trajectory.get_R_world_trunk(t));
}
