...
KinematicsSolver::Priority priority_from_string(std::string priority) {
  if (priority == "soft") {
    ...
    throw std::runtime_error(
        std::string("KinematicsSolver: Invalid priority: ") + priority);
  }
}

KinematicsSolver::Equality::Equality(Eigen::MatrixXd A, Eigen::VectorXd b)
    : A(A), b(b) {}

KinematicsSolver::Objective::Objective(Eigen::MatrixXd P, Eigen::VectorXd q,
                                       double weight)
    : P(P), q(q), weight(weight) {}

KinematicsSolver::KinematicsSolver(MobileRobot &robot) : robot(robot) {
  N = robot.model.nv;
}

void KinematicsSolver::add_position_task(MobileRobot::FrameIndex frame,
...
void KinematicsSolver::add_position_task(std::string frame,
                                         Eigen::Vector3d target_world,
                                         std::string priority, double weight) {
  add_position_task(robot.get_frame_index(frame), target_world,
                    priority_from_string(priority), weight);
}

void KinematicsSolver::add_com_task(Eigen::Vector3d targetCom_world,
                                    Priority priority, double weight) {
...
void KinematicsSolver::add_orientation_task(MobileRobot::FrameIndex frame,
  ...
  Eigen::Vector3d error =
      pinocchio::log3(R_world_target * T_world_frame.linear().inverse());
...
void KinematicsSolver::add_orientation_task(std::string frame,
                                            Eigen::Matrix3d R_world_target,
                                            std::string priority,
                                            double weight) {
  add_orientation_task(robot.get_frame_index(frame), R_world_target,
                       priority_from_string(priority), weight);
}

void KinematicsSolver::add_frame_task(MobileRobot::FrameIndex frame,
                                      Eigen::Affine3d frame_target,
                                      Priority priority, double position_weight,
                                      double orientation_weight) {
  add_position_task(frame, frame_target.translation(), priority,
                    position_weight);
  add_orientation_task(frame, frame_target.rotation(), priority,
                       orientation_weight);
}

void KinematicsSolver::add_frame_task(std::string frame,
                                      Eigen::Affine3d T_world_target,
                                      std::string priority,
                                      double position_weight,
                                      double orientation_weight) {
  add_frame_task(robot.get_frame_index(frame), T_world_target,
                 priority_from_string(priority), position_weight,
                 orientation_weight);
}

void KinematicsSolver::add_pose_task(MobileRobot::FrameIndex frame,
                                     Eigen::Affine3d T_world_target,
                                     Priority priority, double weight) {
  auto T_world_frame = robot.get_T_world_frame(frame);
  ...
  Eigen::VectorXd error =
      pinocchio::log6((T_world_target * T_world_frame.inverse()).matrix())
          .toVector();
...
void KinematicsSolver::add_pose_task(std::string frame,
                                     Eigen::Affine3d T_world_target,
                                     std::string priority, double weight) {
  add_pose_task(robot.get_frame_index(frame), T_world_target,
                priority_from_string(priority), weight);
}
...
Eigen::VectorXd KinematicsSolver::solve(bool apply) {
  ...
  Eigen::MatrixXd CI(0, N); // 2 * constrained_dofs, N);
  Eigen::VectorXd ci0(0); // 2 * constrained_dofs);
  ...
  //   ci0[k * 2] =
  //       -(robot.model.lowerPositionLimit[k + 7] - robot.state.q[k + 7]);
  //   ci0[k * 2 + 1] =
  //       (robot.model.upperPositionLimit[k + 7] - robot.state.q[k + 7]);
  // }
  ...
  double result = eiquadprog::solvers::solve_quadprog(P, q, A.transpose(), -b,
                                                      CI.transpose(), ci0, qd,
...
void KinematicsSolver::create_task(Eigen::MatrixXd A, Eigen::VectorXd b,
                                   Priority priority, double weight) {
  if (priority == Soft) {
    // Ax = b
    ...
    objectives.push_back(
        Objective(A.transpose() * A, -b.transpose() * A, weight));
  } else {
