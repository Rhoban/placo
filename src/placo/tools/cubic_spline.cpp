...
double CubicSpline::pos(double t) { return interpolation(t, Value); }

double CubicSpline::vel(double t) { return interpolation(t, Speed); }

double CubicSpline::acc(double t) { return interpolation(t, Acceleration); }

/**
 * Access to internal Points container
 */
const CubicSpline::Points &CubicSpline::points() const { return _points; }
...
CubicSpline::Polynom CubicSpline::fit(double t1, double x1, double dx1,
                                      double t2, double x2, double dx2) {
  Eigen::Matrix4d M;
  double t1_2 = t1 * t1;
  double t1_3 = t1_2 * t1;
  double t2_2 = t2 * t2;
...
void CubicSpline::compute_splines() {
  ...
  for (size_t i = 1; i < _points.size(); i++) {
    ...
    struct Spline spline = {fit(_points[i - 1].t - t_start, _points[i - 1].x,
                                _points[i - 1].dx, _points[i].t - t_start,
                                _points[i].x, _points[i].dx),
