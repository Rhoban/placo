...
double wrap_angle(double angle) { return atan2(sin(angle), cos(angle)); }
...
Eigen::Affine3d optimal_transformation(Eigen::MatrixXd points_in_A,
                                       Eigen::MatrixXd points_in_B) {
  ...
  if (points_in_A.rows() != points_in_B.rows()) {
    throw std::runtime_error("optimal_transformation(): points in A and B "
