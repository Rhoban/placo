...
bool Segment::is_point_in_segment(const Eigen::Vector2d &point,
                                  double epsilon) {
  Eigen::Vector2d v1 = end - start;
  Eigen::Vector2d v2 = point - start;
  return is_segment_aligned(Segment(start, point), epsilon) &&
         v1.dot(v2) >= 0 && v1.dot(v2) <= v1.dot(v1);
}
