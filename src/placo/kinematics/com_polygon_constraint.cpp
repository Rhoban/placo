...
CoMPolygonConstraint::CoMPolygonConstraint(
    const std::vector<Eigen::Vector2d> &polygon, double margin)
...
void CoMPolygonConstraint::add_constraint(placo::problem::Problem &problem) {
  ...
  if (dcm) {
    if (solver->dt == 0. || omega == 0.) {
      throw std::runtime_error(
          "DCM mode requires a non-zero solver.dt and omega");
    }
  ...
  problem
      .add_constraint(
          problem::PolygonConstraint::in_polygon_xy(com_xy, polygon, margin))
