...
void WheelTask::update() {
  ...
  T_surface_contact.linear().col(0) =
      T_surface_wheel.linear().col(2).cross(Eigen::Vector3d::UnitZ());
  T_surface_contact.linear().col(0).normalize();
  T_surface_contact.linear().col(1) = T_surface_contact.linear().col(2).cross(
      T_surface_contact.linear().col(0));
  T_surface_contact.translation() =
      T_surface_wheel.translation() + radius * downAxis_surface;

  Eigen::Affine3d T_contact_wheel =
      T_surface_contact.inverse() * T_surface_wheel;

  // Computing contact jacobian
  A = (pinocchio::SE3(T_contact_wheel.matrix()).toActionMatrix() *
       solver->robot.joint_jacobian(joint, "local"))
          .topRows(3);
  b = Eigen::Vector3d::Zero();
  b(2, 0) = -T_surface_contact.translation().z();

  // With an omniwheel, we remove the lateral sliding constraint (along contact
  // y axis)
  if (omniwheel) {
...
std::string WheelTask::type_name() { return "wheel"; }

std::string WheelTask::error_unit() { return "m"; }
} // namespace placo::kinematics