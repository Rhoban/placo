...
DistanceConstraint::DistanceConstraint(model::RobotWrapper::FrameIndex frame_a,
...
void DistanceConstraint::add_constraint(placo::problem::Problem &problem) {
  ...
  Eigen::MatrixXd J_a =
      solver->robot.frame_jacobian(frame_a, pinocchio::LOCAL_WORLD_ALIGNED);
  Eigen::MatrixXd J_b =
      solver->robot.frame_jacobian(frame_b, pinocchio::LOCAL_WORLD_ALIGNED);
  Eigen::MatrixXd J_distance =
      direction.transpose() * (J_b - J_a).block(0, 0, 3, solver->N);
