...
PositionTask::PositionTask(model::RobotWrapper::FrameIndex frame_index,
...
void PositionTask::update() {
  ...
  Eigen::MatrixXd J =
      solver->robot.frame_jacobian(frame_index, pinocchio::LOCAL_WORLD_ALIGNED);

  A = mask.apply(J.block(0, 0, 3, solver->N));
  b = mask.apply(error);
}

std::string PositionTask::type_name() { return "position"; }

std::string PositionTask::error_unit() { return "m"; }
} // namespace placo::kinematics