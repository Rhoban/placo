...
ConeConstraint::ConeConstraint(model::RobotWrapper::FrameIndex frame_a,
...
void ConeConstraint::add_constraint(placo::problem::Problem &problem) {
  ...
  Eigen::MatrixXd J_cone =
      T_a_b.linear() *
          solver->robot
              .frame_jacobian(frame_b, pinocchio::ReferenceFrame::LOCAL)
              .block(3, 0, 3, solver->N) -
      solver->robot.frame_jacobian(frame_a, pinocchio::ReferenceFrame::LOCAL)
          .block(3, 0, 3, solver->N);
  ;
  ...
  for (int k = 0; k < N; k++) {
    ...
    Eigen::Matrix3d R_cone_slice =
        Eigen::AngleAxisd(slice_alpha, Eigen::Vector3d::UnitZ())
            .toRotationMatrix();
