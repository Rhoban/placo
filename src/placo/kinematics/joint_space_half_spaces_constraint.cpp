...
JointSpaceHalfSpacesConstraint::JointSpaceHalfSpacesConstraint(
    const Eigen::MatrixXd A, Eigen::VectorXd b)
    : A(A), b(b) {}

void JointSpaceHalfSpacesConstraint::add_constraint(
    placo::problem::Problem &problem) {
  if (A.rows() != b.rows()) {
    throw std::runtime_error("Matrix A and b should have name number of rows "
                             "in joint-space half-spaces constraint");
  }
  if (A.cols() != solver->robot.state.q.rows()) {
    throw std::runtime_error(
        "Matrix A should have ndof cols in joint-space half-spaces constraint");
  }
  ...
  problem.add_constraint(expression <= b)
      .configure(priority == Prioritized::Priority::Hard
