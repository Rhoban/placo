...
ManipulabilityTask::ManipulabilityTask(
    model::RobotWrapper::FrameIndex frame_index, Type type, double lambda)
...
void ManipulabilityTask::update() {
  ...
  Eigen::MatrixXd J_unmasked =
      solver->robot.frame_jacobian(frame_index, pinocchio::LOCAL);
  ...
  for (int dof = 6; dof < solver->N; dof++) {
    Eigen::MatrixXd H_dof_unmasked =
        solver->robot.get_frame_hessian(frame_index, dof);
    Eigen::MatrixXd H_dof = mask_matrix(H_dof_unmasked);
    Eigen::MatrixXd JH = J * H_dof.transpose();

    manipulability_gradient(dof - 6) =
        manipulability * JH.cwiseProduct(JJT_inv).sum();
  }

  // Regularization magnitude is handled through the task weight (see
  // add_regularization_task) Floating base is not regularized by this task
  Eigen::MatrixXd I = Eigen::MatrixXd(solver->N, solver->N);
  I.setIdentity();

  A = Eigen::MatrixXd(solver->N - 6, solver->N);
  A.block(0, 0, solver->N - 6, solver->N) =
      I.block(6, 0, solver->N - 6, solver->N) * lambda;
...
std::string ManipulabilityTask::type_name() { return "manipulability"; }

std::string ManipulabilityTask::error_unit() { return "none"; }
} // namespace placo::kinematics