...
namespace placo {
int JerkPlanner::Constraint::nb_constraints() { return A.rows(); }
...
double JerkPlanner::JerkTrajectory::pos(double t) const {
  ...
  return t * (t * (t * ((1 / 6.) * jerk[k]) + (1 / 2.) * pos_vel_acc[k][2]) +
              pos_vel_acc[k][1]) +
         pos_vel_acc[k][0];
}
...
double JerkPlanner::JerkTrajectory2D::duration() const { return X.duration(); }
...
JerkPlanner::JerkPlanner(int nb_steps, JerkPlanner::State initial_state,
...
Eigen::MatrixXd JerkPlanner::get_transition_matrix(int step) {
  ...
  if (step > 0) {
    transition_matrix.block(0, 0, 6, step * 2) =
        final_transition_matrix.block(0, (N - step) * 2, 6, step * 2);
  }

  return transition_matrix;
}

JerkPlanner::Constraint
JerkPlanner::make_constraint(int step, JerkPlanner::ConstraintType type,
  ...
  for (auto &row : rows) {
    ...
    constraint.A.block(0, 0, 1, N * 2) +=
        row_multiplier * transition_matrix.block(0 + row_offset, 0, 1, N * 2);
    constraint.A.block(1, 0, 1, N * 2) +=
        row_multiplier * transition_matrix.block(3 + row_offset, 0, 1, N * 2);

    constraint.b.block(0, 0, 1, 1) +=
        row_multiplier * (a_powers[step].block(row_offset, 0, 1, 3) *
                          initial_state.block(0, 0, 3, 1));
    constraint.b.block(1, 0, 1, 1) +=
        row_multiplier * (a_powers[step].block(row_offset, 0, 1, 3) *
                          initial_state.block(3, 0, 3, 1));
  }

  return constraint;
}

void JerkPlanner::add_equality_constraint(int step, Eigen::Vector2d value,
                                          JerkPlanner::ConstraintType type) {
  equalities.push_back(make_constraint(step, type, value));
}

void JerkPlanner::add_inequality_constraint(int step, Eigen::Vector2d value,
                                            JerkPlanner::ConstraintType type, bool lower) {
...
void JerkPlanner::add_limit_constraint(double limit,
                                       JerkPlanner::ConstraintType type) {
  for (int step = 0; step < N; step++) {
    // Ax <= b
    add_inequality_constraint(step, Eigen::Vector2d(limit, limit), type, true);

    // Ax >= -b
    add_inequality_constraint(step, -Eigen::Vector2d(limit, limit), type);
  }
}

void JerkPlanner::add_inequality_polygon_constraint(
    int step, std::vector<Eigen::Vector2d> polygon,
    JerkPlanner::ConstraintType type, double margin) {
  Constraint inequality;
  Constraint Ab_matrices = make_constraint(step, type);
  ...
  for (int i = 0; i < polygon.size(); i++) {
    ...
    inequality.A.row(i) = Ab_matrices.A.block(0, 0, 1, N * 2) * n.x() +
                          Ab_matrices.A.block(1, 0, 1, N * 2) * n.y();
    inequality.b.row(i) = Ab_matrices.b.block(0, 0, 1, 1) * n.x() +
                          Ab_matrices.b.block(1, 0, 1, 1) * n.y();
...
int JerkPlanner::nb_constraints(std::vector<Constraint> &constraints) {
  int nb = 0;
...
void JerkPlanner::stack_constraints(std::vector<Constraint> &constraints,
                                    Eigen::MatrixXd &As, Eigen::VectorXd &bs) {
  int n = nb_constraints(constraints);

  As = Eigen::MatrixXd(n, N * 2);
  bs = Eigen::VectorXd(n);
...
JerkPlanner::JerkTrajectory2D JerkPlanner::plan() {
  ...
  Eigen::MatrixXd CE;
  Eigen::VectorXd ce0;
  stack_constraints(equalities, CE, ce0);

  // Stacking inequality constraints
  Eigen::MatrixXd CI;
  Eigen::VectorXd ci0;
  stack_constraints(inequalities, CI, ci0);
  ...
  double result = eiquadprog::solvers::solve_quadprog(
      G, g0, CE.transpose(), ce0, CI.transpose(), ci0, jerks, active_set,
      set_size);

  if (result == std::numeric_limits<double>::infinity()) {
    throw std::runtime_error(
        "CoM planner: Infeasible QP (check your inequality constraints)");
  }
  ...
  for (int step = 0; step < N; step++) {
    ...
    state.block(3, 0, 3, 1) =
        A * state.block(3, 0, 3, 1) + B * jerks[step * 2 + 1];
  }
