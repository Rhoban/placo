...
int Contact::size() { return J.rows(); }

void Contact::add_constraints(Problem &problem) {
  // Forces minimization objective
  if (weight_forces > 0) {
    problem.add_constraint(f == 0).configure(ProblemConstraint::Soft,
                                             weight_forces);
  }
}
...
void PointContact::update() {
  ...
  if (J.rows() != 3) {
    throw std::logic_error("PointContact::update: a point contact should be "
                           "associated with a task of dimension 3 (did "
...
void PointContact::add_constraints(Problem &problem) {
  if (unilateral) {
    ...
    // We don't slip
    problem.add_constraint(f_surface.slice(F_X, 1) <=
                           mu * f_surface.slice(F_Z, 1));
    problem.add_constraint(-mu * f_surface.slice(F_Z, 1) <=
                           f_surface.slice(F_X, 1));

    problem.add_constraint(f_surface.slice(F_Y, 1) <=
                           mu * f_surface.slice(F_Z, 1));
    problem.add_constraint(-mu * f_surface.slice(F_Z, 1) <=
                           f_surface.slice(F_Y, 1));
  }

  // Objective
  if (weight_forces > 0) {
    problem.add_constraint(f.slice(F_X, 3) == 0)
        .configure(ProblemConstraint::Soft, weight_forces);
  }
  if (weight_tangentials > 0) {
    Expression f_surface = R_world_surface.transpose() * f;
    problem.add_constraint(f_surface.slice(F_X, 2) == 0)
        .configure(ProblemConstraint::Soft, weight_tangentials);
  }
}
...
void Contact6D::update() {
  ...
  J.block(0, 0, 3, solver->N) =
      solver->robot.frame_jacobian(position_task->frame_index, pinocchio::LOCAL)
          .block(0, 0, 3, solver->N);
  J.block(3, 0, 3, solver->N) =
      solver->robot
          .frame_jacobian(orientation_task->frame_index, pinocchio::LOCAL)
...
void Contact6D::add_constraints(Problem &problem) {
  if (unilateral) {
    if (length == 0 || width == 0.) {
      throw std::logic_error("Contact length and width should be set for "
                             "unilateral planar contact");
    }
    ...
    problem.add_constraint((-(length / 2) * f.slice(F_Z, 1)) <=
                           f.slice(M_Y, 1));
  ...
  if (weight_tangentials > 0) {
    problem.add_constraint(f.slice(F_Y, 2) == 0)
        .configure(ProblemConstraint::Soft, weight_tangentials);
  }
  if (weight_moments > 0) {
    problem.add_constraint(f.slice(M_X, 3) == 0)
        .configure(ProblemConstraint::Soft, weight_moments);
  }
}

Eigen::Vector3d Contact6D::zmp() {
  return Eigen::Vector3d(-wrench(M_Y, 0) / wrench(F_Z, 0),
                         wrench(M_X, 0) / wrench(F_Z, 0), 0);
}
...
void LineContact::update() {
  ...
  J.block(0, 0, 3, solver->N) =
      solver->robot
          .frame_jacobian(position_task->frame_index,
                          pinocchio::LOCAL_WORLD_ALIGNED)
          .block(0, 0, 3, solver->N);

  if (orientation_task->A.rows() != 2) {
    throw std::logic_error("LineContact::update: an orientation task should be "
                           "associated with a task of dimension 2 "
...
void LineContact::add_constraints(Problem &problem) {
  if (unilateral) {
    if (length == 0) {
      throw std::logic_error(
          "LineContact length should be set for unilateral contact");
    }
    ...
    // We add constraints in the form of:
    // -l_1 f_x <= m_y <= l_1 f_x
    problem.add_constraint(f_surface.slice(M_Y, 1) <=
                           ((length / 2) * f_surface.slice(F_Z, 1)));
    problem.add_constraint((-(length / 2) * f_surface.slice(F_Z, 1)) <=
                           f_surface.slice(M_Y, 1));
  ...
  if (weight_forces > 0) {
    Expression f_surface = R_world_surface.transpose() * f;
    problem.add_constraint(f_surface.slice(F_X, 3) == 0)
        .configure(ProblemConstraint::Soft, weight_forces);
  }
  if (weight_tangentials > 0) {
    Expression f_surface = R_world_surface.transpose() * f;
    problem.add_constraint(f_surface.slice(F_Y, 2) == 0)
        .configure(ProblemConstraint::Soft, weight_tangentials);
  }
  if (weight_moments > 0) {
    // In a line contact, there are only two moments, which are actually M_Y and
    // M_Z
    problem.add_constraint(f.slice(3, 2) == 0)
        .configure(ProblemConstraint::Soft, weight_moments);
  }
}
...
ExternalWrenchContact::ExternalWrenchContact(
...
TaskContact::TaskContact(Task &task) { this->task = &task; }

void TaskContact::update() { J = task->A; }
