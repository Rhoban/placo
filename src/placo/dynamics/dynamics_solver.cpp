...
PointContact &
DynamicsSolver::add_unilateral_point_contact(PositionTask &position_task) {
...
LineContact &
DynamicsSolver::add_unilateral_line_contact(FrameTask &frame_task) {
  return add_contact(new LineContact(frame_task, true));
}

ExternalWrenchContact &DynamicsSolver::add_external_wrench_contact(
...
DynamicsSolver::add_external_wrench_contact(std::string frame_name,
  ...
  if (reference == "world") {
    ...
    throw std::runtime_error(
        "DynamicsSolver::add_external_wrench_contact: unknown reference '" +
        reference + "' (should be 'world' or 'local')");
  }
}
...
PositionTask &
DynamicsSolver::add_position_task(model::RobotWrapper::FrameIndex frame_index,
                                  Eigen::Vector3d target_world) {
  return add_task(new PositionTask(frame_index, target_world));
}

PositionTask &DynamicsSolver::add_position_task(std::string frame_name,
                                                Eigen::Vector3d target_world) {
  return add_position_task(robot.get_frame_index(frame_name), target_world);
}

RelativePositionTask &DynamicsSolver::add_relative_position_task(
    model::RobotWrapper::FrameIndex frame_a_index,
    model::RobotWrapper::FrameIndex frame_b_index, Eigen::Vector3d target) {
  return add_task(
      new RelativePositionTask(frame_a_index, frame_b_index, target));
}

RelativePositionTask &
DynamicsSolver::add_relative_position_task(std::string frame_a_name,
                                           std::string frame_b_name,
                                           Eigen::Vector3d target) {
  return add_relative_position_task(robot.get_frame_index(frame_a_name),
                                    robot.get_frame_index(frame_b_name),
                                    target);
}

RelativeOrientationTask &DynamicsSolver::add_relative_orientation_task(
    model::RobotWrapper::FrameIndex frame_a_index,
    model::RobotWrapper::FrameIndex frame_b_index, Eigen::Matrix3d R_a_b) {
  return add_task(
      new RelativeOrientationTask(frame_a_index, frame_b_index, R_a_b));
}

RelativeOrientationTask &DynamicsSolver::add_relative_orientation_task(
    std::string frame_a_name, std::string frame_b_name, Eigen::Matrix3d R_a_b) {
  return add_relative_orientation_task(robot.get_frame_index(frame_a_name),
                                       robot.get_frame_index(frame_b_name),
                                       R_a_b);
}

RelativeFrameTask DynamicsSolver::add_relative_frame_task(
    model::RobotWrapper::FrameIndex frame_a_index,
    model::RobotWrapper::FrameIndex frame_b_index, Eigen::Affine3d T_a_b) {
  RelativePositionTask &position = add_relative_position_task(
      frame_a_index, frame_b_index, T_a_b.translation());
  RelativeOrientationTask &orientation = add_relative_orientation_task(
      frame_a_index, frame_b_index, T_a_b.rotation());

  return RelativeFrameTask(&position, &orientation);
}

RelativeFrameTask
DynamicsSolver::add_relative_frame_task(std::string frame_a_name,
                                        std::string frame_b_name,
                                        Eigen::Affine3d T_world_frame) {
  return add_relative_frame_task(robot.get_frame_index(frame_a_name),
                                 robot.get_frame_index(frame_b_name),
                                 T_world_frame);
}
...
GearTask &DynamicsSolver::add_gear_task() { return add_task(new GearTask()); }

OrientationTask &DynamicsSolver::add_orientation_task(
    model::RobotWrapper::FrameIndex frame_index,
...
OrientationTask &
DynamicsSolver::add_orientation_task(std::string frame_name,
                                     Eigen::Matrix3d R_world_frame) {
  return add_orientation_task(robot.get_frame_index(frame_name), R_world_frame);
}

FrameTask
DynamicsSolver::add_frame_task(model::RobotWrapper::FrameIndex frame_index,
                               Eigen::Affine3d T_world_frame) {
  PositionTask &position =
      add_position_task(frame_index, T_world_frame.translation());
  OrientationTask &orientation =
      add_orientation_task(frame_index, T_world_frame.rotation());

  return FrameTask(&position, &orientation);
}

FrameTask DynamicsSolver::add_frame_task(std::string frame_name,
                                         Eigen::Affine3d T_world_frame) {
...
DynamicsSolver::~DynamicsSolver() { clear(); }

void DynamicsSolver::enable_joint_limits(bool enable) { joint_limits = enable; }
...
void DynamicsSolver::compute_limits_inequalities(Expression &tau) {
  if ((joint_limits || velocity_limits || velocity_vs_torque_limits) &&
      dt == 0.) {
    throw std::runtime_error(
        "DynamicsSolver::compute_limits_inequalities: dt is not set");
  }
  ...
  if (constraints > 0) {
    ...
    for (int k = 0; k < N - 6; k++) {
      ...
      if (velocity_vs_torque_limits) {
        double ratio =
            robot.model.velocityLimit[k + 6] / robot.model.effortLimit[k + 6];

        // qd + dt*qdd <= qd_max - ratio * tau
        // ratio * tau + dt*qdd + qd - qd_max <= 0
        e.A.block(constraint, 0, 1, problem.n_variables) =
            ratio * tau.A.block(k + 6, 0, 1, problem.n_variables);
      ...
      if (joint_limits) {
        if (q > robot.model.upperPositionLimit[k + 7]) {
          ...
          // qdd*dt + qd <= qd_max
          double qd_max =
              sqrt(2. * (robot.model.upperPositionLimit[k + 7] - q) *
                   qdd_safe[k + 6]);
          e.A(constraint, k + 6) = dt;
        ...
        if (q < robot.model.lowerPositionLimit[k + 7]) {
          ...
          // qdd*dt + qd >= -qd_max
          double qd_max =
              sqrt(2. * fabs(robot.model.lowerPositionLimit[k + 7] - q) *
                   qdd_safe[k + 6]);
          e.A(constraint, k + 6) = -dt;
          e.b(constraint) = -qd - qd_max;
        }
...
void DynamicsSolver::dump_status() { dump_status_stream(std::cout); }
...
DynamicsSolver::Result DynamicsSolver::solve(bool integrate) {
  ...
  // The number of decision variables is now known, resizing the expression of
  // tau
  ...
  // Now, tau = Ax + b with x = [qdd, f1, f2, ...], we copy J^T to the extended
  // A for forces that are decision variables
  int k = N;
  for (auto &contact : contacts) {
    if (contact->active) {
  ...
  for (auto &task : tasks) {
    ...
    if (task->priority == Task::Priority::Soft) {
      ...
      throw std::runtime_error(
          "DynamicsSolver::solve: Scaled priority is not supported");
    }
  ...
  // Floating base has no torque, except if is masked (in that case, the
  // floating base torque will allow to compensate for any motion)
  if (!masked_fbase) {
    problem.add_constraint(tau.slice(0, 6) == 0);
  }

  // We want to minimize actuated torques
  problem.add_constraint(tau.slice(6) == 0)
      .configure(ProblemConstraint::Soft, torque_cost);
  ...
  try {
    ...
    if (integrate) {
      if (dt == 0.) {
        throw std::runtime_error(
            "DynamicsSolver::solve, trying to integrate, but dt is not set");
      }

      robot.state.qdd = result.qdd;
      if (masked_fbase) {
        // Ensuring zero acceleration for the floating base. Even with the hard
        // constraint, numerical issues could yield a drift.
...
void DynamicsSolver::mask_fbase(bool masked) { masked_fbase = masked; }
