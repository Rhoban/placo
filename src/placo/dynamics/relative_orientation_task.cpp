...
RelativeOrientationTask::RelativeOrientationTask(
...
void RelativeOrientationTask::update() {
  ...
  Eigen::MatrixXd Ja =
      solver->robot
          .frame_jacobian(frame_a_index, pinocchio::ReferenceFrame::WORLD)
          .block(3, 0, 3, solver->N);
  Eigen::MatrixXd dJa = solver->robot
                            .frame_jacobian_time_variation(
                                frame_a_index, pinocchio::ReferenceFrame::WORLD)
                            .block(3, 0, 3, solver->N);

  Eigen::MatrixXd Jb =
      solver->robot
          .frame_jacobian(frame_b_index, pinocchio::ReferenceFrame::WORLD)
          .block(3, 0, 3, solver->N);
  Eigen::MatrixXd dJb = solver->robot
                            .frame_jacobian_time_variation(
                                frame_b_index, pinocchio::ReferenceFrame::WORLD)
                            .block(3, 0, 3, solver->N);
  ...
  Eigen::Matrix3d R_a_b_real =
      T_world_a.rotation().transpose() * T_world_b.rotation();
  ...
  Eigen::Vector3d velocity_error_world =
      R_world_a * omega_a_b - world_omega_a_b_real;

  Eigen::Vector3d desired_acceleration = kp * orientation_error_world +
                                         get_kd() * velocity_error_world +
                                         domega_a_b;
  ...
  b = mask.apply(desired_acceleration - Jlog * (dJb * solver->robot.state.qd -
                                                dJa * solver->robot.state.qd));
...
std::string RelativeOrientationTask::error_unit() { return "rad"; }
} // namespace placo::dynamics