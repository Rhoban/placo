...
void RelativePositionTask::update() {
  ...
  Eigen::MatrixXd Ja = solver->robot.frame_jacobian(
      frame_a_index, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED);
  Eigen::MatrixXd dJa = solver->robot.frame_jacobian_time_variation(
      frame_a_index, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED);

  Eigen::MatrixXd Jb = solver->robot.frame_jacobian(
      frame_b_index, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED);
  Eigen::MatrixXd dJb = solver->robot.frame_jacobian_time_variation(
      frame_b_index, pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED);

  // Rotation velocity of frame a in the world frame
  Eigen::Vector3d w_omega_a =
      Ja.block(3, 0, 3, solver->N) * solver->robot.state.qd;
  Eigen::Vector3d a_omega_w = -a_R_w * w_omega_a;

  // Velocity of the error expressed in a
  Eigen::Vector3d w_dAB =
      (Jb.block(0, 0, 3, solver->N) - (Ja.block(0, 0, 3, solver->N))) *
  ...
  Eigen::Vector3d desired_acceleration =
      kp * position_error + get_kd() * velocity_error + ddtarget;

  // The acceleration of AB in a is expressed as: J * ddq + e
  Eigen::MatrixXd J =
      pinocchio::skew(a_AB) * a_R_w * Ja.block(3, 0, 3, solver->N);
  ...
  e += a_R_w * (dJb.block(3, 0, 3, solver->N) - dJa.block(3, 0, 3, solver->N)) *
       solver->robot.state.qd;
  e += pinocchio::skew(a_AB) * a_R_w * dJa.block(3, 0, 3, solver->N) *
       solver->robot.state.qd;
...
std::string RelativePositionTask::type_name() { return "relative_position"; }

std::string RelativePositionTask::error_unit() { return "m"; }
} // namespace placo::dynamics