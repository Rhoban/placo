...
// Overloads
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(frame_configure_overloads, configure, 2,
                                       4);
...
void exposeDynamics() {
  class__<DynamicsSolver::Result>("DynamicsSolverResult")
      ...
      .def(
          "tau_dict",
          +[](const DynamicsSolver::Result &result, RobotWrapper &robot) {
  ...
  class__<PointContact, bases<Contact>>("PointContact",
                                        init<PositionTask &, bool>())
      .def(
          "position_task",
          +[](PointContact &contact) -> PositionTask & {
            return *contact.position_task;
          },
  ...
  class__<Contact6D, bases<Contact>>("Contact6D", init<FrameTask &, bool>())
      .def(
          "position_task",
          +[](Contact6D &contact) -> PositionTask & {
            return *contact.position_task;
          },
          return_internal_reference<>())
      .def(
          "orientation_task",
          +[](Contact6D &contact) -> OrientationTask & {
            return *contact.orientation_task;
          },
          return_internal_reference<>())
  ...
  class__<LineContact, bases<Contact>>("LineContact", init<FrameTask &, bool>())
      .def(
          "position_task",
          +[](LineContact &contact) -> PositionTask & {
            return *contact.position_task;
          },
          return_internal_reference<>())
      .def(
          "orientation_task",
          +[](LineContact &contact) -> OrientationTask & {
            return *contact.orientation_task;
          },
          return_internal_reference<>())
      ...
      .add_property(
          "R_world_surface",
          +[](PointContact &contact) { return contact.R_world_surface; },
          +[](PointContact &contact, Eigen::Matrix3d R) {
            contact.R_world_surface = R;
          });

  class__<ExternalWrenchContact, bases<Contact>>(
      "ExternalWrenchContact", init<RobotWrapper::FrameIndex>())
  ...
  class__<DynamicsSolver>("DynamicsSolver", init<RobotWrapper &>())
      ...
      .add_property(
          "extra_force",
          +[](DynamicsSolver &solver) { return solver.extra_force; },
          +[](DynamicsSolver &solver, Eigen::VectorXd force) {
            solver.extra_force = force;
          })
      .def("mask_fbase", &DynamicsSolver::mask_fbase)
      .def("add_point_contact", &DynamicsSolver::add_point_contact,
           return_internal_reference<>())
      .def("add_unilateral_point_contact",
           &DynamicsSolver::add_unilateral_point_contact,
           return_internal_reference<>())
      .def("add_planar_contact", &DynamicsSolver::add_planar_contact,
           return_internal_reference<>())
      .def("add_fixed_contact", &DynamicsSolver::add_fixed_contact,
           return_internal_reference<>())
      .def("add_line_contact", &DynamicsSolver::add_line_contact,
           return_internal_reference<>())
      .def("add_unilateral_line_contact",
           &DynamicsSolver::add_unilateral_line_contact,
           return_internal_reference<>())
      .def<ExternalWrenchContact &(DynamicsSolver::*)(std::string,
                                                      std::string)>(
          "add_external_wrench_contact",
          &DynamicsSolver::add_external_wrench_contact,
          return_internal_reference<>())
      .def("add_puppet_contact", &DynamicsSolver::add_puppet_contact,
           return_internal_reference<>())
      .def("add_task_contact", &DynamicsSolver::add_task_contact,
           return_internal_reference<>())
      ...
      .def<void (DynamicsSolver::*)(Task &)>("add_task",
                                             &DynamicsSolver::add_task)
      .def<void (DynamicsSolver::*)(Constraint &)>(
          "add_constraint", &DynamicsSolver::add_constraint)
      .def<void (DynamicsSolver::*)(Constraint &)>(
          "add_constraint", &DynamicsSolver::add_constraint)
      .def("clear", &DynamicsSolver::clear)
      .def<void (DynamicsSolver::*)(Task &)>("remove_task",
                                             &DynamicsSolver::remove_task)
      .def<void (DynamicsSolver::*)(FrameTask &)>("remove_task",
                                                  &DynamicsSolver::remove_task)
      ...
      .def(
          "count_contacts",
          +[](DynamicsSolver &solver) { return solver.contacts.size(); })
      .def(
          "get_contact",
          +[](DynamicsSolver &solver, int i) -> Contact & {
            return *solver.contacts[i];
          },
          return_internal_reference<>())
      .def<PositionTask &(DynamicsSolver::*)(std::string, Eigen::Vector3d)>(
          "add_position_task", &DynamicsSolver::add_position_task,
          return_internal_reference<>())
      .def<RelativePositionTask &(DynamicsSolver::*)(std::string, std::string,
                                                     Eigen::Vector3d)>(
          "add_relative_position_task",
          &DynamicsSolver::add_relative_position_task,
          return_internal_reference<>())
      .def<RelativeOrientationTask &(
          DynamicsSolver::*)(std::string, std::string, Eigen::Matrix3d)>(
          "add_relative_orientation_task",
          &DynamicsSolver::add_relative_orientation_task,
          return_internal_reference<>())
      .def<RelativeFrameTask (DynamicsSolver::*)(std::string, std::string,
                                                 Eigen::Affine3d)>(
          "add_relative_frame_task", &DynamicsSolver::add_relative_frame_task)
      .def<JointsTask &(DynamicsSolver::*)()>("add_joints_task",
                                              &DynamicsSolver::add_joints_task,
                                              return_internal_reference<>())
      .def<TorqueTask &(DynamicsSolver::*)()>("add_torque_task",
                                              &DynamicsSolver::add_torque_task,
                                              return_internal_reference<>())
      .def<GearTask &(DynamicsSolver::*)()>("add_gear_task",
                                            &DynamicsSolver::add_gear_task,
                                            return_internal_reference<>())
      .def<CoMTask &(DynamicsSolver::*)(Eigen::Vector3d)>(
          "add_com_task", &DynamicsSolver::add_com_task,
          return_internal_reference<>())

      .def<OrientationTask &(DynamicsSolver::*)(std::string, Eigen::Matrix3d)>(
          "add_orientation_task", &DynamicsSolver::add_orientation_task,
          return_internal_reference<>())
      .def<FrameTask (DynamicsSolver::*)(std::string, Eigen::Affine3d)>(
          "add_frame_task", &DynamicsSolver::add_frame_task);

  class__<Task, bases<tools::Prioritized>, boost::noncopyable>("DynamicsTask",
                                                               no_init)
  ...
  class__<PositionTask, bases<Task>>(
      "DynamicsPositionTask", init<RobotWrapper::FrameIndex, Eigen::Vector3d>())
  ...
  class__<OrientationTask, bases<Task>>(
      ...
      init<RobotWrapper::FrameIndex, Eigen::Matrix3d>())
  ...
  class__<Constraint, bases<tools::Prioritized>, boost::noncopyable>(
      "DynamicsConstraint", no_init);

  class__<AvoidSelfCollisionsConstraint, bases<Constraint>>(
      "AvoidSelfCollisionsDynamicsConstraint", init<>())
