...
// Returns the exposed name of a given type
template <typename T>
std::string exposed_name()
{
  ...
  {
    PyTypeObject* obj = reg->get_class_object();
    if (obj != NULL)
    {
      return obj->tp_name;
    }
...
// Appending [type] in the docstring
template <typename T>
const char* typed_docstring(std::string doc)
{
  std::string full_doc = doc + " [returns " + exposed_name<T>() + "]";
...
// Adding numpy type (for eigen structs)
